                ...
                match destination {
                    ...
                    RealizedVariable::Offset { base, offsets, items } => {
                        ...
                        for offset in offsets.iter().take(offsets.len() - 1) {
                            if offset.internal_size_bytes_pow2() > 0 {
                                writer.write_all(format!("\tsalq ${}, {}\n", offset.internal_size_bytes_pow2(), offset.offset.to_string()).as_bytes())?;
                            }
                            writer.write_all(format!("\taddq {}, %r15\n", offset.offset.to_string()).as_bytes())?;
                            writer.write_all(b"\tmovq (%r15), %r15\n")?;
                        }

                        let last_offset = offsets.last().unwrap();

                        if last_offset.internal_size_bytes_pow2() > 0 {
                            writer.write_all(format!("\tsalq ${}, {}\n", last_offset.internal_size_bytes_pow2(), last_offset.offset.to_string()).as_bytes())?;
                        }
                        writer.write_all(format!("\taddq {}, %r15\n", last_offset.offset.to_string()).as_bytes())?;
                        writer.write_all(format!("\tmov{} %r14{}, (%r15)\n\tpopq %r14\n", if last_offset.internal_size_bytes == 1 { "b" } else { "q" }, if last_offset.internal_size_bytes == 1 { "b" } else { "" }).as_bytes())?;
                        if last_offset.internal_size_bytes == 1 {
                            writer.write_all(b"\tandq $0xff, (%r15)\n")?;
                        }
                        writer.write_all(b"# done offset\n")?;
                    },
                }
...
impl VariableSizeInfo {
    /// Since the value is already a pointer (if this is being called), then you don't need to include it in the number of derefs needed
    fn as_ind_vec(&self) -> Vec<usize> {
        let mut init = vec![self.size];
        init.resize(self.indirection, 8);
...
struct LocalVariable {
    location: Box<dyn Memory>,
    size: VariableSizeInfo,
}

/// Handle potentially multiple globals
fn handle_global_decl(pair: Pair<'_, Rule>, globals: &mut HashMap<String, VariableSizeInfo>) -> Vec<ProgramItem> {
    let mut pairs = pair.into_inner();
    let ty = pairs.next().unwrap().into_inner().next().unwrap();

    // Quick and dirty
    let size = if ty.as_str().contains("char") { 1 } else { 8 };
    ...
    decl_ident_list
        ...
        .map(|ident| {
            ...
            if globals.iter().any(|g| g.0 == ident.as_str()) {
                panic!(
            ...
            globals.insert(ident.clone(), VariableSizeInfo { size, indirection });
...
struct RealizedVariableOffsetInfo {
    ...
    /// If this is of type `char`, it is 1, 8 for all other cases (`long`, any pointer)
    /// Determines whether a shift (multiplication) is needed on the dereference
    internal_size_bytes: usize,
}

impl RealizedVariableOffsetInfo {
    fn internal_size_bytes_pow2(&self) -> usize {
        (self.internal_size_bytes as f64).log2() as usize
    }
}
...
enum RealizedVariable {
    ...
    Offset {
        ...
        offsets: Vec<RealizedVariableOffsetInfo>,
...
fn handle_function(
    pair: Pair<'_, Rule>,
    globals: &HashMap<String, VariableSizeInfo>,
    ...
    let arg_list_str = if has_args_list {
        arg_list
            ...
            .map(|arg| {
                ...
                let ty = arg.next().unwrap().into_inner().next().unwrap();
                ...
                (arg.next().unwrap().as_str().to_string(), VariableSizeInfo { size, indirection })
            })
            .collect()
    } else {
        Vec::new()
    ...
    let mut local_stack = HashMap::<String, LocalVariable>::new();
    let mut memory = MemoryManager::default();

    let mut arg_mems = Vec::new();
    for (arg, size_info) in arg_list_str {
        // Allocate stack memory since &ident may happen
        let mem = memory.next_free_stack_memory();
        local_stack.insert(arg, LocalVariable { location: mem.to_owned(), size: size_info });
        arg_mems.push(mem);
    }
...
fn extract_children(statements: Option<Pair<'_, Rule>>, memory: &mut MemoryManager, mut local_stack: HashMap<String, LocalVariable>, globals: &HashMap<String, VariableSizeInfo>, strings: &mut Vec<String>, fn_name: &str, cfsi: Option<ControlFlowStateInformation>) -> Vec<ProgramItem> {
...
fn handle_sub_scope(pair: Pair<'_, Rule>, memory: &mut MemoryManager, local_stack: HashMap<String, LocalVariable>, globals: &HashMap<String, VariableSizeInfo>, strings: &mut Vec<String>, fn_name: &str, cfsi: Option<ControlFlowStateInformation>) -> Vec<ProgramItem> {
    match pair.as_rule() {
        Rule::while_expr => {
            let mut pairs = pair.into_inner();
            let jump = JumpDestination::new();
            let mem = memory.next_free_stack_memory();
            let expr_items = handle_expression(pairs.next().unwrap(), memory, &local_stack, globals, RealizedVariable::Memory(mem.to_owned()), strings);

            let children = extract_children(pairs.next(), memory, local_stack, globals, strings, fn_name, Some(cfsi.unwrap_or(ControlFlowStateInformation { break_name: jump.end.clone(), continue_name: jump.then.clone() })));
        ...
        Rule::do_while_expr => {
            ...
            let children = extract_children(pairs.next(), memory, local_stack.clone(), globals, strings, fn_name, Some(cfsi.unwrap_or(ControlFlowStateInformation { break_name: jump.end.clone(), continue_name: jump.then.clone() })));
            let expr_items = handle_expression(pairs.next().unwrap(), memory, &local_stack, globals, RealizedVariable::Memory(mem.to_owned()), strings);
        ...
        Rule::for_expr => {
            let mut pairs = pair.into_inner();
            let init_var = handle_assignment(pairs.next().unwrap(), memory, &local_stack, globals, strings);
            let condition_result = memory.next_free_stack_memory();
            let condition = handle_expression(pairs.next().unwrap(), memory, &local_stack, globals, RealizedVariable::Memory(condition_result.to_owned()), strings);
            let incrementor = handle_assignment(pairs.next().unwrap(), memory, &local_stack, globals, strings);

            let jump = JumpDestination::new();

            let children = extract_children(pairs.next(), memory, local_stack, globals, strings, fn_name, Some(cfsi.unwrap_or(ControlFlowStateInformation { break_name: jump.end.clone(), continue_name: jump.incr.clone() })));
        ...
        Rule::if_expr => {
            ...
            let children = extract_children(pairs.next(), memory, local_stack.clone(), globals, strings, fn_name, cfsi.clone());
            let else_children = extract_children(pairs.next().map(|p| p.into_inner().next().unwrap()), memory, local_stack, globals, strings, fn_name, cfsi);
...
fn handle_function_call(
    ...
    local_stack: &HashMap<String, LocalVariable>,
    globals: &HashMap<String, VariableSizeInfo>,
    strings: &mut Vec<String>,
) -> Vec<ProgramItem> {
    let mut pairs = pair.into_inner();
...
fn handle_local_decl(
    pair: Pair<'_, Rule>,
    memory: &mut MemoryManager,
    local_stack: &mut HashMap<String, LocalVariable>,
    globals: &HashMap<String, VariableSizeInfo>,
) {
    let mut pairs = pair.into_inner();
    let ty = pairs.next().unwrap().into_inner().next().unwrap();
    ...
    for ident in decl_ident_list.into_inner() {
        ...
        // Allocate stack memory since &ident may happen
        local_stack.insert(ident.as_str().to_string(), LocalVariable { location: memory.next_free_stack_memory(), size: VariableSizeInfo { size, indirection } });
    }
}
...
fn handle_assignment(
    ...
    let rv = match dest.as_rule() {
        Rule::array_access => {
            ...
            let ident = pairs.next().unwrap();
            let (base, size) = get_variable_from_ident(local_stack, ident, globals);
            let mut size_outside_in = size.as_ind_vec();
            size_outside_in.reverse();

            let offset_expressions = pairs.collect::<Vec<_>>();

            let (offsets, items): (Vec<_>, Vec<_>) = offset_expressions.into_iter().zip(size_outside_in).map(|(expression, size)| {
                ...
                (RealizedVariableOffsetInfo { offset: res, internal_size_bytes: size }, out)
            }).unzip();
            
            RealizedVariable::Offset { base: Box::new(base), offsets, items: items.into_iter().flatten().collect() }
        }
        Rule::ident_name => get_variable_from_ident(local_stack, dest, globals).0,
        _ => unreachable!(),
    };
...
fn get_variable_from_ident(local_stack: &HashMap<String, LocalVariable>, ident: Pair<'_, Rule>, globals: &HashMap<String, VariableSizeInfo>) -> (RealizedVariable, VariableSizeInfo) {
    local_stack
        .iter()
        .find(|(lsv, _)| lsv.as_str() == ident.as_str().trim())
        .map(|(_, m)| (RealizedVariable::Memory(m.location.to_owned()), m.size))
        .or_else(|| {
            globals
                .iter()
                .find(|g| g.0.as_str() == ident.as_str().trim())
                .map(|g| (RealizedVariable::Global(g.0.to_owned()), *g.1))
        })
...
impl Operation {
    fn write<W: Write>(
        ...
        match self {
            ...
            Operation::And => writer.write_all(
                format!("\tandq {}, {}\n", arg1.to_string(), arg2.to_string()).as_bytes(),
            ),
            Operation::Mult => {
                writer
...
fn handle_primary_expression(
    ...
    match pair.as_rule() {
        ...
        Rule::array_access => {
            ...
            let size_data_loc = if let Some((_, local_mem)) = local_var {
                info.stack.push(ExpressionInstruction { 
                    op: Operation::Mov, 
                ...
                info.stack.push(ExpressionInstruction { 
                    op: Operation::Sub, 
                    arg1: Operand::IntegerConstant(local_mem.location.stack_offset() as i64), 
                    arg2: Operand::Memory(Box::new(CalleeSavedRegister::R15)) 
                });
            ...
            let global_var = globals.iter().find(|g| g.0.as_str() == ident.as_str().trim());

            let size_data_glob = if let Some(global) = global_var {
                info.stack.push(ExpressionInstruction {
                    op: Operation::Mov,
                    arg1: Operand::Global(global.0.to_owned()),
                    arg2: Operand::Memory(Box::new(CalleeSavedRegister::R15)),
                });
                info.stack.push(ExpressionInstruction {
                    op: Operation::Mov,
            ...
            let size_data = size_data_glob.or(size_data_loc.as_ref()).unwrap();
            let mut size_data = size_data.as_ind_vec();
            // Reverse since derferencing arrays will take off pointers then get to the final type
            size_data.reverse();
            ...
            for (expression, size) in offset_expressions.into_iter().zip(size_data.into_iter()) {
                ...
                if size == 1 {
                    info.stack.push(ExpressionInstruction { op: Operation::And, arg1: Operand::IntegerConstant(0xff), arg2: Operand::Memory(Box::new(CalleeSavedRegister::R15)) });
                }
            }
        ...
        Rule::ident => {
            ...
            if let Some((_, local_mem)) = local_var {
                ...
                if address_of {
                    ...
                    info.stack.push(ExpressionInstruction {
                        op: Operation::Mov,
                        arg1: Operand::Memory(local_mem.location.to_owned()),
                        arg2: Operand::Memory(mem.to_owned()),
                    });
                }
            ...
            let global_var = globals.iter().find(|g| g.0.as_str() == pair.as_str().trim());
