...
use std::{process::exit, fs, io::{Write, BufWriter}, collections::{HashSet, VecDeque}, hash::Hash, fmt::Debug};
...
const REGISTER_STACK: [CalleeSavedRegister; 5] = [CalleeSavedRegister::Rbx, CalleeSavedRegister::R10, CalleeSavedRegister::R13, CalleeSavedRegister::R14, CalleeSavedRegister::R15];
...
const ARGUMENT_REGISTERS: [ArgumentRegister; 6] = [ArgumentRegister::Rdi, ArgumentRegister::Rsi, ArgumentRegister::Rdx, ArgumentRegister::Rcx, ArgumentRegister::R8, ArgumentRegister::R9];
...
impl ProgramItem {
    fn write<W: Write>(&self, writer: &mut W) -> Result<(), std::io::Error> {
        match self {
            ...
            ProgramItem::Function { name, args, children } => {
                // Header
            ...
            Self::FunctionCall { name, args } => {
                assert!(args.len() <= ARGUMENT_REGISTERS.len(), "More args than supported for function `{name}`");
            ...
            Self::StringLiteralLoad { id, target } => {
                writer.write_all(format!("\tmovq $global_string_{id}, %{}", target.to_string()).as_bytes())?;
...
fn main() {
    ...
    program.into_inner().flat_map(|pair| {
        match pair.as_rule() {
...
fn handle_global_decl(pair: Pair<'_, Rule>, globals: &mut HashSet<Variable>) -> Vec<ProgramItem> {
    ...
    let num_indirection = ty.filter(|pair| matches!(pair.as_rule(), Rule::pointer)).count();

    let decl_ident_list = pairs.next().unwrap();

    decl_ident_list.into_inner().map(|ident| {
        let ident = ident.as_str().to_string();

        if globals.iter().any(|g| g.name == ident.as_str()) {
            panic!("Variable redeclaration error! Variable `{}` is defined more than once.", ident.as_str());
        }
...
fn handle_function(pair: Pair<'_, Rule>, globals: &HashSet<Variable>, strings: &mut Vec<String>) -> ProgramItem {
    let mut pairs = pair.into_inner();
    let ident = pairs.nth(1).unwrap().as_str();

    let arg_list = pairs.next();

    let has_args_list = arg_list.clone().is_some_and(|pair| pair.into_inner().any(|p| matches!(p.as_rule(), Rule::function_arg)));
    let args_list_is_statements = arg_list.is_some() && !has_args_list;

    let arg_list_str = if has_args_list {
        arg_list.clone().unwrap().into_inner().map(|arg| {
    ...
    let children = if let Some(statements) = statements {
        statements.into_inner().flat_map(|statement| {
    ...
    ProgramItem::Function { name: ident.to_string(), args: arg_list_str, children }
}

fn handle_local_decl(pair: Pair<'_, Rule>, local_stack: &mut Vec<Variable>, globals: &HashSet<Variable>) {
    let mut pairs = pair.into_inner();
    let ty = pairs.next().unwrap().into_inner();
    ...
    for ident in decl_ident_list.into_inner() {
        if globals.iter().any(|g| g.name == ident.as_str()) || local_stack.iter().any(|l| l.name == ident.as_str()) {
            panic!("Variable redeclaration error! Variable `{}` is defined more than once.", ident.as_str());
        }

        local_stack.push(Variable { name: ident.as_str().to_string(), indirection: num_indirection })
    }
}
...
fn handle_assignment(pair: Pair<'_, Rule>, strings: &mut Vec<String>) -> Vec<ProgramItem> {
    ...
    handle_expression(pairs.next().unwrap(), Box::new(ArgumentRegister::R8), strings);
    todo!()
}
...
fn handle_expression(pair: Pair<'_, Rule>, destination: Box<dyn Memory>, strings: &mut Vec<String>) -> ProgramItem {
...
fn build_expression_stack(pair: Pair<'_, Rule>, info: &mut ExpressionInfo, strings: &mut Vec<String>) -> Box<dyn Memory> {
    match pair.as_rule() {
...
fn handle_primary_expression(pair: Pair<'_, Rule>, info: &mut ExpressionInfo, strings: &mut Vec<String>) -> Box<dyn Memory> {
    info.depth += 1;
    ...
    match pair.as_rule() {
        Rule::literal => {
            ...
            match lit.as_rule() {
                Rule::string => {
                    ...
                    info.stack.push(ExpressionInstruction { op: Operation::Mov, arg1: Operand::StringConstant(strings.len() - 1), arg2: Operand::Register(dyn_clone::clone_box(next_loc.as_ref())) });
                    next_loc
                },
                Rule::integer => {
                    let next_loc = info.next_open_memory_location();
                    info.stack.push(ExpressionInstruction { op: Operation::Mov, arg1: Operand::IntegerConstant(lit.as_str().trim().parse().unwrap()), arg2: Operand::Register(dyn_clone::clone_box(next_loc.as_ref())) });
                    next_loc
                },
                _ => unreachable!()
            }
        ...
        Rule::expression => build_expression_stack(pair.into_inner().next().unwrap(), info, strings),
        Rule::ident => todo!(),
        _ => unreachable!(),
    }
}

fn four_function_variadic_expression_helper(pair: Pair<'_, Rule>, info: &mut ExpressionInfo, strings: &mut Vec<String>) -> Box<dyn Memory> {
    let mut pairs = pair.into_inner();
    if pairs.len() == 1 {
...
fn eq_or_rel_expression_helper(pair: Pair<'_, Rule>, info: &mut ExpressionInfo, strings: &mut Vec<String>) -> Box<dyn Memory> {
    let mut pairs = pair.into_inner();
    if pairs.len() == 1 {
        return build_expression_stack(pairs.next().unwrap(), info, strings);
    }
...
fn variadic_expression_helper(pair: Pair<'_, Rule>, info: &mut ExpressionInfo, strings: &mut Vec<String>, op: Operation) -> Box<dyn Memory> {
    let mut pairs = pair.into_inner();
    if pairs.len() == 1 {
        return build_expression_stack(pairs.next().unwrap(), info, strings);
    }
    ...
    for op_mem in ops {
        info.stack.push(ExpressionInstruction { op, arg1: Operand::Register(op_mem), arg2: Operand::Register(dyn_clone::clone_box(result_reg.as_ref())) })
    }
